...
class TransformInYawPlane:
    ...
    def __call__(self, sample):
        ...
        if self.reflections:
            ...
            P = torch.eye(3).unsqueeze(0).expand(N,-1,-1).clone().to(sample["ts_us"].device)
            P[indices] = P[indices]@ torch.Tensor([[0,1,0],[1,0,0],[0,0,1]]).to(sample["ts_us"].device)
            ...
            for sensor, feat in feats_new.items():
                ...
                if "imu" in sensor:
                    assert feat.shape[1] == 6
                    feat_aug[indices, :3, :] = -1 * feat_aug[indices, :3, :]
