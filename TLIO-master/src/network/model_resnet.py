...
class ResNet1D(nn.Module):
    ...
    def __init__(
        ...
        self.output_block2 = FcBlock(512 * block_type.expansion, cov_output_dim, inter_dim)
...
if __name__ == "__main__": ## to quickly check
    input = torch.randn((1,6,200)).to('cuda').to(torch.float32)
    network = ResNet1D(
                BasicBlock1D, 6, 3, [2, 2, 2, 2], 200//32 +1
            )
    network = network.to('cuda').to(torch.float32)
    network.eval()
    print(input.device)
