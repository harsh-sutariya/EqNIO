...
for idx in range(len(n_rot)):
    ...
    for rot in angle_list:
        ...
        for folder in data_list:
            ...
            if os.path.isdir(path+folder+'_ref_aug'+str(int(rot))+str(0+idx)):
                print(path+folder+'_ref_aug'+str(int(rot))+str(0+idx)+'folder exists')
            else:
                os.mkdir(path+folder+'_ref_aug'+str(int(rot))+str(0+idx))
            aug_file_list.append(folder+'_ref_aug'+str(int(rot))+str(0+idx))
            
            ## copy imu0_resampled_description.npy
            shutil.copy(path+folder+'/imu0_resampled_description.json',path+folder+'_ref_aug'+str(int(rot))+str(0+idx)+'/imu0_resampled_description.json')
            ...
            if os.path.exists(path+folder+'/imu0_resampled.npy'):
                ...
                np.save(path+folder+'_ref_aug'+str(int(rot))+str(0+idx)+'/imu0_resampled.npy', np.concatenate([ts.reshape((-1,1)), gyro, accel, q_ori, pos, vel], axis=1))
            ...
            final_R = R@P
            accelBias = -1*np.einsum('ji,jk->ik',final_R,accelBias)
            gyroBias = np.einsum('ji,jk->ik',final_R,gyroBias)
            ## For rectification matrix it is R.T(GR) when G is in inertial frame
            accelScaleInv = np.einsum('ji,jk->ik',final_R,np.einsum('ij,jk->ik',accelScaleInv,final_R))
            gyroScaleInv = np.einsum('ji,jk->ik',final_R,np.einsum('ij,jk->ik',gyroScaleInv,final_R))
            ...
            with open(path+folder+'_ref_aug'+str(int(rot))+str(0+idx)+'/calibration.json', "w") as outfile: 
    ...
    with open(path +'test_list_ref_aug'+str(idx)+'.txt', "w") as output:
...
for i in range(5):
    data_list =[]
    with open(path +'test_list_ref_aug'+str(i)+'.txt') as f:
        data_list = [s.strip() for s in f.readlines() if len(s.strip()) > 0]
        if len(data_list)>500:
            for j in range(0,len(data_list),500):
                with open(path +'test_list_ref_aug'+str(i)+'part'+str(j)+'.txt', "w") as output:
                    end_j = j+500
