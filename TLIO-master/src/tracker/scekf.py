...
class ImuMSCKF:
    ...
    def is_mahalanobis_activated(self):
        ...
        if self.last_success_mahalanobis is not None:
            if self.state.s_timestamp_us > self.last_success_mahalanobis + int(
                ...
                logging.warning(
                    "Deactivating Mahalanobis test, because failed for too long." ##---------------------> when it fails for too long you force an updateuntil some timestep - 5 steps?
                )
                self.last_success_mahalanobis = None
    ...
    def update(self, meas, meas_cov, t_begin_us, t_end_us):
        ...
        if self.yaw_aligned: #local yaw aligned frame
            # print('Entered measurement update local gravity aligned frame')
        ...
        elif self.local_frame: #local frame
            # print('Entered measurement update local frame')
            Ri = self.state.si_Rs[begin_idx]
            ri_eul = compute_euler_from_matrix(Ri, "xyz", extrinsic=True)  # in radians
            ri_y = ri_eul[0, 1]
            ri_z = ri_eul[0, 2]
            ...
            H[:, (6 * begin_idx + 3) : (6 * begin_idx + 6)] = -Ri_z.T
            H[:, (6 * end_idx + 3) : (6 * end_idx + 6)] = Ri_z.T
            H[:, (6 * begin_idx) : (6 * begin_idx + 3)] = hat(self.state.si_ps[end_idx] - self.state.si_ps[begin_idx])
        
        else: # world frame
            # print('Entered measurement update global frame')
            Ri = self.state.si_Rs[begin_idx]
            ri_eul = compute_euler_from_matrix(Ri, "xyz", extrinsic=True)  # in radians
            ri_y = ri_eul[0, 1]
            ri_z = ri_eul[0, 2]
            ...
            H = np.zeros((3, 15 + 6 * self.state.N))
            H[:, (6 * begin_idx + 3) : (6 * begin_idx + 6)] = -1
            H[:, (6 * end_idx + 3) : (6 * end_idx + 6)] = 1
            H[:, (6 * begin_idx) : (6 * begin_idx + 3)] = 0
        ...
        if self.mahalanobis_factor > 0:
            ...
            Sinv_temp = np.linalg.inv(S_temp)          ##-----------------------------------------here it fails!
        ...
        Sinv = np.linalg.inv(S)##-----------------------------------------here it fails!
